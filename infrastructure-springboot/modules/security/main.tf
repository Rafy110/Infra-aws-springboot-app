...
# Allow ECS to read Secrets Manager secrets (for database password)
...
# Policy for ECS Exec (allows exec into containers)
...
# IAM User for GitHub Actions (for CI/CD)
resource "aws_iam_user" "github" {
  name = "${var.app_name}-${var.environment}-github-user"
  path = "/"
}

# Policy for GitHub Actions to push to ECR and update ECS
resource "aws_iam_user_policy" "github" {
  name = "${var.app_name}-${var.environment}-github-policy"
